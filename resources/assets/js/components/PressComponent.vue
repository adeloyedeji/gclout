        ...
        <div class="box profile-info n-border-top" id="press_block">
            ...
            <div class="box-footer box-form">
                <div class="pull-right">
                    <button type="button" class="btn btn-azure" id="postBtn" :disabled="has_content" @click="create_post()">Press Release</button>
                </div>
    ...
    mounted() {
        ...
        this.dropzone.on('success', function (file, response) {
            if(response == "unable to save post") {
                ...
                vm.content = ''
                vm.$store.commit("update_press", response)
                vm.removeAllFiles()
            }
        })
    ...
    methods: {
        create_post() {
            ...
            if(this.dropzone.files.length) {
                ...
                axios.post('/press/store', {body:this.content})
                     .then( (resp) => {
                         ...
                         if(resp.data === -1) {
                            ...
                            this.$store.commit("update_press", resp.data)
                            // this.$store.commit("update_press_photos", resp.data.photos)
                            this.content = ''
                             //$("#press_block_content").append(resp.data)
                         }
                     }).catch(error => {
                        new Noty({
                            type: 'error',
