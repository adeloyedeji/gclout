<template>
    <div>
        <div class="modal fade" id="uploadCoverBox" tabindex="-1" role="dialog" aria-labelledby="uploadAvatarBox" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title" id="uploadAvatarBox">Change Cover Photo</h4>
                    </div>
                    <div class="modal-body">
                        <div class="dropzone" id="coverbox">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                        <button type="button" id="changeAvatar" class="btn btn-primary" @click.prevent="save_cover">Update Cover Photo</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted() {
        const element = document.getElementById("coverbox")
        ...
        this.dropzone.on('success', function (file, response) {
            if(response == 1) {
                ...
                new Noty({
                    type: 'warning',
                    layout: 'bottomLeft',
                    text: 'Unable to cover photo at this time!'
                }).show();
            } else {
                new Noty({
                    type: 'warning',
